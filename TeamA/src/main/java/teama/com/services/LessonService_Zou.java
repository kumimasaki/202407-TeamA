...
@Service
public class LessonService_Zou {
	@Autowired
	private LessonDao_Zou lessonDaoZou;

	public List<Lesson> selectAllLessonList(Long adminId) {
		if (adminId == null) {
			return null;
		} else {
			return lessonDaoZou.findAll();
		}
	}
	...
	public Lesson lessonEditCheck(Long lessonId) {
		if (lessonId == null) {
			...
			return lessonDaoZou.findByLessonId(lessonId);
		}
	}
	...
	public boolean imageUpdate(String imageName) {
		if (imageName == null) {
			...
			Lesson img = lessonDaoZou.findByImageName(imageName);
			img.setImageName(imageName);
			lessonDaoZou.save(img);
			return true;
		}
	}
